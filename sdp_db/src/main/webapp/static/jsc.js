    ...
    .then(
      function(response) {
        ...
        response.json().then(function(data) {
          var date = new Date();
          //date.setHours(08,30);  // showing real time
            ...
            if((((hours == start_lesson[0]) && (mins >= start_lesson[1])) ||
              (hours > start_lesson[0])) && (((hours == end_lesson[0]) &&
               (mins <= end_lesson[1])) || (hours < end_lesson[0]))){
                  ...
                  cur_lesson.innerHTML = "<span class='room_num'>"+data[k].room+"</span>" + "<br>"
  ...
  var myHeaders = new Headers();
    ...
    function getData(room){
      fetch('/sdp_db/jd?room='+room, myInit)
      .then(
        function(response) {
          if (response.status !== 200) {
            alert('Server Error. Status Code: ' + response.status);
          ...
          response.text().then(function(data) {
             document.getElementById('response').innerHTML = "<span class='dataBase'>" + "<img src='static/info.png' class='ig'>" + data + "</span>";
              });
  ...
  if(parameter !== null){
    ...
    document.getElementById('screen').innerHTML = "<center>Please add room number. Example:room=A102</center>";
  }
